...
const Login = () => {
  ...
  const handleToggleLogin = () => setToggleLogin((prev) => !prev);
  ...
  // We still use cookies to store ONLY the auth token (no email).
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [cookies, setCookie, removeCookie] = useCookies<any>(["AuthToken"]);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // ✅ Store ONLY the token, not the email
      setCookie("AuthToken", response.token, { path: "/issue-tracker-lite" });
